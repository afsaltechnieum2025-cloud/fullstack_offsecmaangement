...
import { Search, Plus, ExternalLink, ImageIcon, X, FileText } from 'lucide-react';

export default function Trending() {
  const [searchQuery, setSearchQuery] = useState('');
  const [notes, setNotes] = useState<any[]>([]);
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ name: '', link: '', description: '', photo: null as File | null, photoPreview: null as string | null });
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => setForm(f => ({ ...f, photo: file, photoPreview: ev.target?.result as string }));
    reader.readAsDataURL(file);
  };

  const handleSubmit = () => {
    if (!form.name.trim() || !form.description.trim()) return;
    setNotes(prev => [{ id: Date.now(), ...form }, ...prev]);
    setForm({ name: '', link: '', description: '', photo: null, photoPreview: null });
    setOpen(false);
  };

  const filtered = notes.filter(n => n.name.toLowerCase().includes(searchQuery.toLowerCase()));
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0 space-y-1.5">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {note.link && (
                          <a href={note.link} target="_blank" rel="noopener noreferrer" className="shrink-0 text-muted-foreground hover:text-primary transition-colors">
